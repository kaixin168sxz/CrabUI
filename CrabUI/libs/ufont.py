    ...
    def init(self, string) -> int:
        ...
        for char in string:
            if char not in font_list:
                font_list[char] = framebuf.FrameBuffer(bytearray(list(self.get_bitmap(char))),
                                                       font_size, font_size, framebuf.MONO_HLSB)
            # 英文字符半格显示
            w += half_font_size if ord(char) < 128 else font_size
        self.w = w
        return w
